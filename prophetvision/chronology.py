...
# ---------------------------------------------------------------------------
# Screen layout (1206x910 reference frames)
# ---------------------------------------------------------------------------
...
class DigitOCR:
    ...
    def __init__(self, templates_path: str | None = None):
        ...
        else:  # fallback: synthetic bold sans-serif glyphs
...
class Chronology:
    ...
    def _countdown(self, t: float, frame: np.ndarray) -> SpinEvent | None:
        box = frame[COUNTDOWN_Y0:COUNTDOWN_Y1,
                    COUNTDOWN_X0:COUNTDOWN_X1].astype(np.int16)
        if box.size == 0:
            return None
        mn = box.min(axis=2)
    ...
    def _bets_close(self, t: float, frame: np.ndarray) -> SpinEvent | None:
        band = frame[BETS_Y0:BETS_Y1, BETS_X0:BETS_X1].astype(np.int16)
    ...
    def _find_marker(self, frame: np.ndarray):
        ...
        m[:35, :] = 0  # ignore the history banner
        n, lab, stats, _ = cv2.connectedComponentsWithStats(m, 8)
        best = None
        for i in range(1, n):
            x, y, w, h, a = stats[i]
            if a < 175 or not (10 <= w <= 45) or not (13 <= h <= 45):
                continue
            if not (0.35 <= w / h <= 1.8):
                continue
    ...
    def read_banner(self, frame: np.ndarray) -> list[int]:
        ...
        strip = frame[BANNER_Y0:BANNER_Y1].astype(np.int16)
